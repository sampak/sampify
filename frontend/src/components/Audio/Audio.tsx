...
function Audio(){
  const [volume, setVolume] = useState(parseFloat(localStorage.getItem('audio_volume') ?? '0.5'));
  ...
  const onClickPrevious = () => {
    ...
    const song = getPreviouslySong(playlists, playerSong.activePlaylistGuid, playerSong.activeSongGuid);
    if(!song) {
      player.current.audio.current.currentTime = 0;
      return;
    }
  ...
  const onClickNext = () => {
    const song = getNextSong(playlists, playerSong.activePlaylistGuid, playerSong.activeSongGuid);
    if(!song){
      return;
    }
    MusicHook.fetch(song.guid, playerSong.activePlaylistGuid);
